...
bool Log::isValidIDFormat(const std::string &id)
{
    ...
    if (id.length()
        > 6) { // Limiting log ID to 6 characters for simplicity just chose this amount arbitrarily, can be changed if needed
...
{
    qDebug() << "Log[id =" << logID << ", date=" << date.toString("yyyy-MM-dd hh:mm:ss")
